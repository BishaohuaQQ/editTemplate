  ...
  <div class="create-activity">
    ...
    <div class="main">
      <div class="page-controller">
        <router-link class="item-control" :to="{ path: '/' }"><el-button type="primary">活动中心</el-button></router-link>
        <router-link class="item-control" :to="{ path: '/activity_list' }"><el-button type="primary">已创建活动</el-button></router-link>
      </div>
        ...
        <div class="rule-config">
          <el-form :inline="true" :model="ruleConfig" class="demo-form-inline">
            <el-form-item label="活动标题" required>
              <el-input v-model="ruleConfig.activityTitle" placeholder="请输入活动标题"></el-input>
            </el-form-item>
            <el-form-item label="活动时间" required>
              <el-col :span="11">
                <el-date-picker v-model="ruleConfig.date" value-format="yyyy-MM-dd HH:mm" type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
              </el-col>
            </el-form-item>
            <br />
            <el-form-item label="选择媒体" required>
              <el-radio-group v-model="ruleConfig.media" size="medium">
              	<el-button :class="{ activeClass : isActive === 1}"  @click="selectedClass(1)">默认全部</el-button>
								<el-button :class="{ activeClass : isActive === 2}"  @click="selectedClass(2)">定投({{this.dingtouarr.length}})</el-button>
								<el-button :class="{ activeClass : isActive === 3}"  @click="selectedClass(3)">屏蔽({{this.pingbiarr.length}})</el-button>
                <!--<el-radio border label="默认全部" @change="centerDialogVisible =true"></el-radio>
                <el-radio border label="定投" @change="centerDialogVisible =true"></el-radio>
                <el-radio border label="屏蔽" @change="centerDialogVisible =true"></el-radio>-->
              </el-radio-group>
            </el-form-item>
            <el-form-item label="活动次数">
              <el-input v-model="ruleConfig.frequency" placeholder="请输入活动次数"></el-input>
            </el-form-item>
            <el-form-item label="活动排序">
              <el-input v-model="ruleConfig.sort" suffix-icon="el-icon-sort" placeholder="请输入活动排序"></el-input>
            </el-form-item>
          </el-form>
        </div>
      </div>
              ...
              <el-row>
                <el-col :span="20" class="edit-list">
                  ...
                  <div class="operate-activity">
                    <div class="btn-wrap">
                      <el-button size="small" plain>返回上一步</el-button>
                      <el-button size="small" type="primary" @click="saveActivity">保存并发布</el-button>
                    </div>
                  </div>
                </el-col>
                <el-col :span="4" class="edit-detail">
                  <div class="edit-detail-top">
                    <el-button type="text" icon="iconfont icon-back" class="revoke">撤销修改</el-button>
                  </div>
                  <div class="edit-detail-body">
                    <div class="edit-box" v-if="editType === 'img'">
                      ...
                      <el-upload class="upload-demo" action="http://test6.plat.adjuz.net/Iad_CustomerTemplate/UploadImg" accept="image/jpeg,image/png" :on-success="uploadSuccess" :on-error="uploadError" :on-progress="progress" :file-list="uploadImg.fileList" :on-remove="delImg">
                        ...
                        <el-button size="small" type="primary" :disabled="uploadImg.imgLoad">点击上传</el-button>
                      </el-upload>
                    </div>
    ...
    <el-dialog  :visible.sync="centerDialogVisible" :show-close= "false" width="700px" height="580px" center>
      ...
      <div class="frame-header">
	        <h4>选择广告位</h4>
	        <span class="el-icon-close" @click="centerDialogVisible=false"></span>
      ...
      <div>
      	<ul class="pro-ul">
      		...
      		<li v-for="(item , index) in tempMediaList" class="pro-li">
...
//				设置定投,屏蔽初始个数，要先判断是不是为空
 ...
 // 如果模板存在
    ...
    saveActivity () {
      ...
      // 活动标题不能为空
      if (!title) {
        this.errorPrompt('warning', '活动标题不能为空！')
        return
      }
      // 活动有效期时间
...
<style lang="scss">
  ...
  .activity-config{
    margin-bottom: 100px;
    .phone-model{
      position: relative; width: 340px; height: 640px; margin: 0 auto; background: url(phone.png) no-repeat 0 0; background-size: contain;
      .view-phone{
